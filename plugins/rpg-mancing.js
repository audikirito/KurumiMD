...
${ usedPrefix }Cook catfish `    
        ...
        conn.reply( m.chat, mcng, m)
         ...
         conn.sendButton( m.chat, `You Get 🎁chest weapons epic ${psepick} item`, `Kurumi-MD`, [[`again`, `.mancing`]], m)
         } 
        if  (psenjata > 0 ) {
        	global.db.data.users[m.sender].psenjata+= psenjata * 1
        conn.sendButton( m.chat, `You Get 🎁chest weapons ${psenjata} item`, `Kurumi-MD`, [[`Again`, `.mancing`]], m)
        	}
        }, 38000)
